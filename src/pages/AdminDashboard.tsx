...
export default function AdminDashboard() {
  ...
  const confirmedFounding = useMemo(() => profiles.filter((p) => p.role === "artist" && p.founding_artist).length, [profiles]);
  const remaining = Math.max(0, FOUNDING_CAP - confirmedFounding);

  // Track pending "founding" toggle per application row (before it becomes a profile)
