...
const EditorialPage = () => (
  <main className="pt-14">
    ...
    <section className="px-6 md:px-12 pt-16 pb-12 bg-white">
      <div className="eyebrow mb-3">Pensar la escultura</div>
      ...
      <p className="font-body text-[16px] font-light text-gray max-w-[640px] mt-4">
        Ensayos, reportajes y entrevistas en torno a la escultura, el oficio y su mercado.
      </p>
    </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14">
        {piezas.map(p => (
          <article key={p.titulo} className="group cursor-pointer">
            ...
            <h2 className="font-display font-bold text-[clamp(20px,2vw,26px)] tracking-[-0.02em] text-ink leading-tight mb-3">
              {p.titulo}
            </h2>
            <p className="font-body text-[15px] font-light text-gray leading-relaxed mb-3">{p.extracto}</p>
            <div className="font-body text-[12px] uppercase tracking-[0.14em] text-muted-line">Por {p.autor}</div>
          </article>
        ))}
      </div>
    </section>
